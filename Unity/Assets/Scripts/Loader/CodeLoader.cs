	...
	public class CodeLoader: Singleton<CodeLoader>
	{
		...
		void StartGame()
		{
			...
			foreach (var item in HotfixDll)
			{
				...
				if (item == "Model.dll")
				{
					Debug.Log($"Model is set Unity.Model.dll {assembly.GetName().Name}");
					this.model = assembly;
				}
			...
			{
				//Debug.Log($"StartGame EventSystemData:{item.Key} Length:{item.Value}");
			}
			
			Debug.Log("Start Game Begin!!!! are you ready?");
			Type type = typeof(StaticMethod);
			Debug.Log($"StaticMethod is Ready {type.Namespace},{type.Name}");

			IStaticMethod start = new StaticMethod(this.model, "ET.Entry", "Start");
			
			Debug.Log($"Start is {start.GetType().ToString()}");
			start.Run();
		}
